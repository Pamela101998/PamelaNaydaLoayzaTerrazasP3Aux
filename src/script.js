...
function myfunction  () {
    const registerNewAnswer = poll;
    registerNewAnswer = poll.bind (prompt(poll.question + poll.options));
...
const y = 0;
y =  parseInt(registerNewAnswer);

if (y = 0 && y <= 3){
    poll.answers = answers.length [0] + 1;
} else if (y = 1 && y <= 3){
    poll.answers = answers.length [1] + 1;
} else if (y = 2 && y <= 3){
    poll.answers = answers.length [2] + 1;
} else if (y = 3 && y <= 3){
    poll.answers = answers.length [3] + 1;
} else {
    alert("Sólo se permite valores de 0 a 3");
}
...
const type = [ ];
const displayResults = type.poll.answers;
